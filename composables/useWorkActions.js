...
export function useWorkActions(context) {
	...
	const startWork = async (hours, wage) => {
		...
		messageList.value.push({
			role: 'system',
			content: `💼 (你离开了家，前往公司开始 ${hours} 小时的辛勤工作...)`,
			isSystem: true
		});
		await saveHistory();
	...
	const finishWork = async (hours, wage) => {
		...
		if (!char.economy) char.economy = {
			wallet: 0,
			courierBox: [],
			containers: {}
		};

		// ✨ 修改：优先把打工赚的钱加到玩家的独立钱包中
		if (char.economy.userWallet !== undefined) {
			char.economy.userWallet += wage;
		} else {
			char.economy.wallet += wage; // 兼容极老的存档
		}

		// ✨ 同步总金额，防止依赖旧版 wallet 的地方出错
		char.economy.wallet = char.economy.isSharedWallet ?
			((Number(char.economy.userWallet) || 0) + (Number(char.economy.charWallet) || 0)) :
			(char.economy.userWallet !== undefined ? char.economy.userWallet : char.economy.wallet);
		...
		messageList.value.push({
			role: 'system',
			content: `💸 (工作结束！赚取了 ¥${wage}，当前余额: ¥${char.economy.wallet}。时间来到了 ${formattedTime.value})`,
			isSystem: true
		});
		await saveHistory();
	...
	const goHome = async (oldTimeStr) => {
		...
		nextTick(() => {
			const workPrompt =
				`[SYSTEM EVENT: BACK FROM WORK] 玩家刚才去公司辛苦打工了，现在终于下班回到了家(${playerLocation.value})。时间从 ${oldTimeStr} 跳转到了 ${formattedTime.value}。请结合你们现在的关系和时间，对辛苦赚钱回家的玩家说一句极其自然、贴心的话（可以问问他累不累，或者聊聊晚饭吃什么）。`;
