...
export function useGameLocation(context) {
	...
	const locationList = computed(() => {
		...
		// 获取当前的地址状态
		const pLoc = playerLocation?.value || currentRole.value?.playerLocation || userHome.value;
		const cHome = charHome.value || '角色家';
		const uHome = userHome.value || '玩家家';

		// ✨ 核心修改：判定玩家当前是否在角色家里（或者两人同居）
		// 如果 pLoc 是 customRooms 的其中一个，或者等于角色家地址，说明玩家已经进屋了
		const isPlayerInCharHome = customRooms.includes(pLoc) || pLoc === cHome || pLoc === '她的家';
		
		// 只有同居，或者玩家已经移动到角色家里，房间子场景才可用
		...
		if (isTogether) {
			...
			// 如果不同居，且玩家不在她的家，才显示去“她的家”的选项
			if (!isPlayerInCharHome) {
				list.push({
					name: '她的家',
					detail: charHome.value || '角色家',
					type: 'char_home',
					icon: '🏠',
					mode: 'face', // 移动过去后变为 face
					style: 'background-color:#fff0f5; color:#d81b60; border:1px solid #ffcdd2;'
				});
			}
			
			// 如果玩家不在自己家，显示回“我的家”的选项
			if (pLoc !== uHome && pLoc !== '我的家') {
				list.push({
					name: '我的家',
					detail: userHome.value || '我家',
					type: 'user_home',
					icon: '🏡',
					mode: 'phone', // 回自己家后变为 phone
