<template>
	<view class="footer">
		...
		<view class="edit-toolbar" v-if="isEditMode">
			...
			<view class="delete-confirm-btn" @click="$emit('confirmDelete')" :class="{ 'active': selectedCount > 0 }">删除
			</view>
		</view>
		...
		<view class="input-container" v-else>
			...
			<view class="camera-popup" v-if="showCameraMenu" @click.stop>
				...
				<view class="camera-actions">
					...
					<view class="action-item" @click="$emit('clickGodView')">
						<text class="icon">👁️</text>
						<text class="label">显影</text>
					</view>
				</view>
			</view>
			...
			<view class="toolbar-compact" v-if="isToolbarOpen">
				<scroll-view class="tool-scroll" scroll-x="true" show-scrollbar="false">
					<view class="tool-flex">
						...
						<picker v-if="!isEmbedded" mode="time" :value="wakeTime" start="00:00" end="23:59"
						...
						<view class="tool-item" @click="toggleCameraMenu" :class="{ active: showCameraMenu }"
							v-if="!isEmbedded">
							<view class="tool-icon">📷</view>
						...
						<view class="tool-item" @click="$emit('clickInvite')" v-if="showInviteAction">
							<view class="tool-icon">邀</view>
			...
			<view class="input-area">
				...
				<input class="input" :value="modelValue" @input="$emit('update:modelValue', $event.detail.value)"
					confirm-type="send" @confirm="$emit('send')" placeholder="输入对话..." />
